...
function saveStats(){localStorage.setItem("egeAiStats",JSON.stringify(stats));}
function saveSolvedTasks(){localStorage.setItem("solvedTasks",JSON.stringify(solvedTasks));}
...
function updateStatsUI(){
  ...
  if(list){
    ...
    subjects.forEach(function(subject){
      ...
      div.innerHTML="<b>"+subject+"</b><span>Решено: "+item.solved+"</span><span>Верно: "+item.correct+"</span><span>Точность: "+accuracy(item.correct,item.solved)+"%</span>";
...
function updateDependentFilters(){
  const source=currentSubject==="all"?tasks:tasks.filter(function(task){
...
function updateTaskCount(){
  ...
  if(!el)return;
  el.textContent="Найдено заданий: "+filteredTasks.length+" · Уже решено: "+solvedTasks.length;
}
